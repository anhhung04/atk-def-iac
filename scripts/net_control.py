...
INIT_RULES = [
    ...
    "INPUT -p udp --dport 51820 -j ACCEPT",  # wireguard listeners
...
def add_teams_arguments(command_parser):
    ...
    teams_group.add_argument(
        "--range", type=str, metavar="N-N", help="Range of teams (inclusive)"
    )
    teams_group.add_argument(
        "--list", type=str, metavar="N,N,...", help="List of teams"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--verbose", "-v", help="Turn verbose logging on", action="store_true"
    )
    parser.add_argument(
        "--dry-run", help="Just print rules (verbose mode)", action="store_true"
    )
    ...
    shutdown_parser = subparsers.add_parser(
        "shutdown", help="Remove all the added rules"
    )
    ...
    unban_parser.add_argument(
        "--team", type=int, metavar="N", help="Team number for unban"
    )
    ...
    isolate_parser.add_argument(
        "--team", type=int, metavar="N", help="Team number for isolation"
    )
