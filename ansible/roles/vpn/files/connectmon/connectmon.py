...
TEAM_NAMES = collections.defaultdict(
    str,
    {
        0: "jury",
    },
...
SERVICE_NAMES = collections.defaultdict(
    str,
    {
        80: "http",
    },
...
def count_connections_from_conntrack():
    p = subprocess.Popen(
        CONNTRACK_ARGS, encoding="utf8", stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
...
def handle_client(cl):
    ...
    pkt_body_list = []
    pkt_body_list.append(
        "# HELP ctf_connects The number of established connections between teams"
    )
    ...
    for src_team, dst_team, dst_service in connect_cnt:
        ...
        dst_service_metric_name = 'dst_service="%d %s"' % (
            dst_service,
            SERVICE_NAMES[dst_service],
        )

        metric = "ctf_connects{%s,%s,%s} %d" % (
            src_team_metric_name,
            dst_team_metric_name,
            dst_service_metric_name,
            val,
        )
...
def print_team_info(team):
    ...
    for src_team, dst_team, dst_service in connect_cnt:
        ...
        out_msg = "to team %d %s service %d %s: %d" % (
            dst_team,
            dst_team_name,
            dst_service,
            dst_service_name,
    ...
    for src_team, dst_team, dst_service in connect_cnt:
        ...
        in_msg = "from team %d %s service %d %s: %d" % (
            src_team,
            src_team_name,
            dst_service,
            dst_service_name,
